  ...
  app_container = merge(
    {
      ...
      #
      # OPENAI_API_KEY と AUTH_PASSWORD は共有基盤のシークレットをそのまま使う。
      # 共有基盤が古くて output がまだ無い場合は null で渡ってくるので、その時は
      # 環境変数ごと落とす (どちらもアプリ側で既定値がある)。
      secrets = concat(
        [
          {
        ...
        var.auth_password_secret_arn == null ? [] : [
          {
            name      = "AUTH_PASSWORD"
